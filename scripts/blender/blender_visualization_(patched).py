...
def _coord_key_is_neuron(k):
    """Exclude time keys; all other keys are treated as neuron ids (string digits)."""
    if k in ("/t", "t"):
        return False
    return True


def _build_voltage_key_index(raw_activity):
    """
    Map neuron id -> exact NPZ key for that trace.
    Supports '/voltages/<id>', 'voltages/<id>', and any future slash prefix variant.
    """
    index = {}
    for k in raw_activity.keys():
        if k in ("/t", "t"):
            continue
        m = re.match(r"^/?voltages/(\d+)$", k)
        if m:
            nid = int(m.group(1))
            index[nid] = k
    return index
...
def _resolve_num_frames(raw_activity, voltage_key_index, neuron_ids, activity_path=""):
    """Length of simulation axis: prefer explicit time array, else first available voltage trace."""
    for tk in ("/t", "t"):
        if tk in raw_activity:
            return int(np.asarray(raw_activity[tk]).shape[0])
    for nid in neuron_ids:
        vk = voltage_key_index.get(nid)
        if vk is not None:
            return int(np.asarray(raw_activity[vk]).shape[0])
    if voltage_key_index:
        vk = next(iter(voltage_key_index.values()))
        return int(np.asarray(raw_activity[vk]).shape[0])
    loc = f" ({activity_path})" if activity_path else ""
    raise ValueError(
        "No time array ('t' or '/t') and no voltage traces in activity file" + loc
    )


def _get_voltage_array(raw_activity, nid, voltage_key_index):
    """Return (array, key) or (None, None)."""
    vk = voltage_key_index.get(nid)
    if vk is None:
        for cand in (f"/voltages/{nid}", f"voltages/{nid}"):
            if cand in raw_activity:
                vk = cand
                break
    if vk is None:
        return None, None
    return np.asarray(raw_activity[vk], dtype=np.float32), vk


def load_data(ACTIVITY_PATH, POSITION_PATH):
    raw_coords = np.load(POSITION_PATH)
    raw_activity = np.load(ACTIVITY_PATH)
    neuron_ids = sorted([int(k) for k in raw_coords.keys() if _coord_key_is_neuron(k)])
    voltage_key_index = _build_voltage_key_index(raw_activity)
    num_frames = _resolve_num_frames(
        raw_activity, voltage_key_index, neuron_ids, activity_path=ACTIVITY_PATH
    )
    ...
    for i, nid in enumerate(neuron_ids):
        arr, vk = _get_voltage_array(raw_activity, nid, voltage_key_index)
        if arr is not None:
            n = min(num_frames, arr.shape[0])
            activity_matrix[:n, i] = arr[:n]
            activity_max[i] = float(arr.max())
            activity_min[i] = float(arr.min())
        else:
            print(f"warning: no voltage trace for neuron id {nid} (tried keys like voltages/{nid} and /voltages/{nid})")
